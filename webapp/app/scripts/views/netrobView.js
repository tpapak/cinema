...
// netrobView.js — Network risk of bias view using hyperscript-helpers
//
// Replaces netrob.hbs Handlebars template.
// Pure function: (data) => VTree
// data = { text, statusReady, ruleName, customized, customizedSingular,
//          numberCustomized, rulesselections, boxes }
...
// Shared box rendering for netrob and netindr
// Resolve any nested function properties on box objects (from view.js lazy getters)
var renderBox = (box, text, statusReady, actionNs) => {
  ...
  return li('.well.compRobSelector' + (customized ? '.customized' : ''), {
    ...
    ul([
      ...
      li([
        ...
        select('.mr-sm-2.controls-container', {
          onchange: function() { Actions[actionNs].selectIndividual(this); },
...
var netrobView = (data) => {
  ...
  if (!statusReady) {
    ...
    headerContent.push(
      h4('.controls-title.mr-sm-2', [
        ...
        span('.info.glyphicon.glyphicon-info-sign', {
          ...
          onclick: function() {
            Actions.alertify().message('Choosing "Majority RoB" will lead to a level of concern according to the risk of bias with the greatest total percentage contribution (the greatest block between green, yellow, and red in each bar). The "Highest RoB" will assign a level of concern determined by the highest risk of bias in each bar. Summarizing risk of bias assessments using "Average RoB" uses a weighted average score for each relative effect estimate according to the percentage contribution of studies at each bias level. Details can be found in Section 4.1 of the detailed manual.');
          },
        }),
      ])
    );
    headerContent.push(
      select('.mr-sm-2.form-inline.form-control', {
        onchange: function() { Actions.NetRob.selectRule(this); },
        disabled: statusReady,
      }, ruleOptions)
    );
  }
  ...
  if (statusReady) {
    ...
    headerContent.push(
      button('.btn.btn-default', {
        onclick: function() { Actions.NetRob.resetNetRob(); },
      }, text.resetButton || 'Reset')
    );
    headerContent.push(
      button('.btn.btn-default.btn-pad', {
        onclick: function() { Actions.NetRob.proceed(); },
      }, 'Proceed')
    );
  }
  ...
  var visibleBoxes = totalBoxes > MAX_VISIBLE_BOXES ? boxes.slice(0, MAX_VISIBLE_BOXES) : boxes;
  var boxItems = visibleBoxes.map(function(box) {
    return renderBox(box, text, statusReady, 'NetRob');
  });
  ...
  return [
    h('blockquote#netRob.col-xs-12', [
      div('.form-inline', headerContent),
    ]),
    div('#netRobSelector.col-xs-12.well.collapse.in', boxContent),
  ];
};

module.exports = netrobView;
