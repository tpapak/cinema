...
instance decodeTreatmentIdInstance :: DecodeJson TreatmentId where
  decodeJson tid =
    let
      sid = decodeJson tid :: Either JsonDecodeError String
    in
      case sid of
        Left _ ->
          let
            iidResult = decodeJson tid :: Either JsonDecodeError Int
          in
            case iidResult of
...
stringToComparison del str = do
  ...
  if (length sid == 2) then
    let
      st1 = stringToTreatmentId
...
isIdOfComparison id comp =
  let
    t1 = min (comp ^. _Comparison)."t1" (comp ^. _Comparison)."t2"
...
sortStringComparisonIds fsids =
  let
    eids = decodeJson fsids :: Either JsonDecodeError (Array String)
    ids = case eids of
      Left _ -> []
      Right idss -> idss
  in
...
fixComparisonId fsid =
  let
    esid = decodeJson fsid :: Either JsonDecodeError String
    sid = case esid of
      Left _ -> "error"
...
orderIds ftids =
  let
    eids = decodeJson ftids :: Either JsonDecodeError (Array TreatmentId)
    res = case eids of
      Left _ -> ftids
...
instance decodeInterventionType :: DecodeJson InterventionType where
  decodeJson json = case toObject json of
    ...
    Just obj -> do
      ...
      is <- getField obj "isSelected"
      ia <- getField obj "isActive"
      id <- getField obj "isDisabled"
      pure $ InterventionType { id: i, label: l, isSelected: is, isActive: ia, isDisabled: id }
...
instance decodeNode :: DecodeJson Node where
  decodeJson json = case toObject json of
    ...
    Just obj -> do
      ...
      let
        l = case labelResult of
          Left _ ->
            let
              numLabelResult = getField obj "label" :: Either JsonDecodeError Int
            in
              case numLabelResult of
                Left _ -> ""
      ...
      let
        interventionType = case itResult of
