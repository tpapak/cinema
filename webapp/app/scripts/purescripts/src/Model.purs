...
instance decodeState :: DecodeJson State where
  decodeJson json = case toObject json of
    ...
    Just obj -> do
      proj <- getField obj "project"
      txt <- getField obj "text"
      pure $ State { project: proj, text: txt }

getState :: Json -> Either JsonDecodeError State
getState json = case toObject json of
  Nothing -> Left $ TypeMismatch "Object"
  Just obj -> do
    proj <- getField obj "project"
    txt <- getField obj "text"
...
instance decodeStudies :: DecodeJson Studies where
  decodeJson json = case toObject json of
    Nothing -> Left $ TypeMismatch "Object"
    Just obj -> do
      dc <- getField obj "directComparisons"
      ic <- getField obj "indirectComparisons"
      n <- getField obj "nodes"
      pure $ Studies { directComparisons: dc, indirectComparisons: ic, nodes: n }
...
directComparisons
  :: forall a b r
   . Lens { directComparisons :: a | r } { directComparisons :: b | r } a b
directComparisons = prop (Proxy :: Proxy "directComparisons")

indirectComparisons
  :: forall a b r
   . Lens { indirectComparisons :: a | r } { indirectComparisons :: b | r } a b
...
instance decodeCMContainer :: DecodeJson CMContainer where
  decodeJson json = case toObject json of
    Nothing -> Left $ TypeMismatch "Object"
    Just obj -> do
      cm <- getField obj "currentCM"
      pure $ CMContainer { currentCM: cm }
...
instance decodeContributionMatrix :: DecodeJson ContributionMatrix where
  decodeJson json = case toObject json of
    ...
    Just obj -> do
      ...
      cn <- getField obj "colNames"
      drn <- getField obj "directRowNames"
      irn <- getField obj "indirectRowNames"
      p <- getField obj "params"
      sc <- getField obj "selectedComparisons"
      pure $ ContributionMatrix { status: st, colNames: cn, directRowNames: drn, indirectRowNames: irn, params: p, selectedComparisons: sc }
...
instance decodeCMParameters :: DecodeJson CMParameters where
  decodeJson json = case toObject json of
    Nothing -> Left $ TypeMismatch "Object"
    Just obj -> do
      ma <- getField obj "MAModel"
      inv <- getField obj "intvs"
      r <- getField obj "rule"
      s <- getField obj "sm"
      pure $ CMParameters { "MAModel": ma, intvs: inv, rule: r, sm: s }
