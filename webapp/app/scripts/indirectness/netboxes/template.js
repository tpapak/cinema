var View = require('./view.js')();
var h = require('virtual-dom/h');
// var VNode = require('vtree/vnode');
// var VText = require('vtree/vtext');
// var convertHTML = require('html-to-vdom')({
//      VNode: VNode,
//      VText: VText
// });
var netindrView = require('../../views/netindrView.js');

var resolveGetters = require('../../lib/mixins.js').resolveGetters;

var Template = (model,children) => {
    // var tmpl = GRADE.templates.netindr(
    //   _.extend(View(model),{text:model.getState().text.NetIndr})
    // );
    // return h('div#NetIndr.col-xs-12',convertHTML(tmpl));
    var data = _.extend(resolveGetters(View(model)), { text: model.getState().text.NetIndr });
    return h('div#NetIndr.col-xs-12', netindrView(data));
}

module.exports = () => {
  return Template;
}
